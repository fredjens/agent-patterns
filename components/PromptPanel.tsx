...
export function PromptPanel({ prompt, running, onPromptChange, onRun, onAbort }: Props) {
  return (
    <div className="border-t border-zinc-800 bg-zinc-950 p-4 flex flex-col gap-3">

      {!prompt && <div className="flex flex-wrap gap-1.5">
        {EXAMPLE_PROMPTS.map((p) => (
          <button
            key={p}
            disabled={running}
            onClick={() => onPromptChange(p)}
            className="rounded-full border border-zinc-700 bg-zinc-900 px-3 py-1 text-xs text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200 hover:border-zinc-600 transition-colors disabled:opacity-40 disabled:cursor-not-allowed cursor-pointer"
          >
            {p}
          </button>
        ))}
